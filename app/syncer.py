...
def _do_sync(task_id):
    ...
    logger.info(f"[Sync {task_id}] Starting — triggered by: {_get_triggered_by(task_id)}")
    ...
    # Step 2: refresh WebDAV snapshot if needed
    ...
    if age is None or age > ttl_hours:
        ...
        snapshot = database.get_snapshot_paths()
        logger.info(f"[Sync {task_id}] Step 2: using cached snapshot "
    ...
    for local_path, local_size in local_files.items():
        ...
        if overwrite_policy == "skip_if_same_size" and local_size == remote_size:
            skipped += 1
            continue
    ...
    logger.info(f"[Sync {task_id}] Need to upload: {len(to_upload)}, skip: {skipped}")
    ...
    database.update_sync_task(task_id, status="uploading",
                              done=enqueued, finished_at=_now())
    logger.info(f"[Sync {task_id}] Enqueued {enqueued} files, sync task done")
    database.update_sync_task(task_id, status="done", finished_at=_now())
...
def _fetch_webdav_snapshot(cfg, remote_dir):
    ...
    result = {}
    _propfind_recursive(target_url, auth, remote_dir, result)
    return result


def _propfind_recursive(url, auth, base_remote, result, depth=0):
    if depth > 10:
        return
    try:
    ...
    for response in root.findall("D:response", ns):
        ...
        if href_path == base_path or href_path == base_path + "/":
            continue

        if is_dir:
            child_url = f"{parsed.scheme}://{parsed.netloc}{href_path}/"
            _propfind_recursive(child_url, auth, base_remote, result, depth + 1)
        else:
...
def _get_triggered_by(task_id):
    task = database.get_last_sync_task()
    return task.get("triggered_by", "unknown") if task else "unknown"
