...
function saveSection(section) {
  ...
  if (section === 'webdav') {
    ...
    payload = {
      concurrent_uploads: parseInt(document.getElementById('cfg-concurrent').value),
      overwrite_policy: document.querySelector('input[name="ow"]:checked').value,
      retry_count: parseInt(document.getElementById('cfg-retry').value),
      single_file_timeout_minutes: parseInt(document.getElementById('cfg-timeout').value),
    };
  } else if (section === 'schedule') {
    payload = {
      scheduler_time: document.getElementById('cfg-sched-time').value.trim(),
      sync_speed_limit: document.getElementById('cfg-speed').value,
      webdav_snapshot_ttl_hours: parseInt(document.getElementById('cfg-ttl').value),
      log_retain_days: parseInt(document.getElementById('cfg-logdays').value),
    };
  }
...
// ── 标签管理 ──────────────────────────────────────────────
function renderTags(containerId, values, type) {
  const el = document.getElementById(containerId);
  if (!el) return;
  
  el.innerHTML = values.map(v =>
    `<span class="tag">${escapeHtml(v)}<span class="tag-x" onclick="removeTag('${containerId}','${escapeHtml(v)}','${type}')">×</span></span>`
  ).join('') + `<span class="tag tag-add" onclick="addTag('${containerId}','${type}')">+ 添加</span>`;
}

function getTagValues(containerId) {
  const el = document.getElementById(containerId);
  if (!el) return [];
  return Array.from(el.querySelectorAll('.tag:not(.tag-add)')).map(t => t.childNodes[0].textContent.trim());
}

function addTag(containerId, type) {
  const val = prompt(type === 'ext' ? '输入扩展名（如 .mkv）：' : '输入忽略目录名：');
  if (!val || !val.trim()) return;
  const current = getTagValues(containerId);
  if (!current.includes(val.trim())) current.push(val.trim());
  renderTags(containerId, current, type);
}

function removeTag(containerId, val, type) {
  const current = getTagValues(containerId).filter(v => v !== val);
  renderTags(containerId, current, type);
}
...
function testConnections() {
  ...
  const webdavStatus = document.getElementById('conn-webdav');
  const dirStatus = document.getElementById('conn-dir');
  const rcloneStatus = document.getElementById('conn-rclone');
  ...
  fetch('/api/test/webdav', {
    ...
    if (webdavStatus) {
      webdavStatus.innerHTML = d.ok
        ? '<span style="color:#0F6E56">● 正常</span>'
        : `<span style="color:#A32D2D">● ${escapeHtml(d.error || '失败')}</span>`;
    }
  }).catch(() => {
  ...
  if (dirPath) {
    fetch('/api/test/localdir', {
      ...
      if (dirStatus) {
        dirStatus.innerHTML = d.ok
          ? `<span style="color:#0F6E56">● 可读 (${d.file_count} 个文件)</span>`
          : `<span style="color:#A32D2D">● ${escapeHtml(d.error || '失败')}</span>`;
      }
    }).catch(() => {
      if (dirStatus) dirStatus.innerHTML = '<span style="color:#A32D2D">● 目录不可读</span>';
    });
  } else {
    if (dirStatus) dirStatus.innerHTML = '<span style="color:#aaa">● 未配置</span>';
  }

  if (rcloneStatus) rcloneStatus.innerHTML = '<span style="color:#0F6E56">● 已就绪</span>';
}
...
function testWebdav() {
  ...
  fetch('/api/test/webdav', {
    ...
    if (resultSpan) {
      resultSpan.innerHTML = d.ok
        ? '<span style="color:#0F6E56">● 连接正常</span>'
        : `<span style="color:#A32D2D">● ${escapeHtml(d.error || '失败')}</span>`;
    }
  });
}
...
function testDir() {
  const path = document.getElementById('cfg-watch-dir').value.trim();
  const resultSpan = document.getElementById('test-dir-result');
  if (resultSpan) resultSpan.innerHTML = '测试中…';
...
function showToast(msg, isErr) {
  ...
  Object.assign(t.style, {
    ...
    background: isErr ? '#c0392b' : '#1a1a18', color: '#fff',
